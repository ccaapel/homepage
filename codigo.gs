...
function reordenarTudo() {
  ...
  ABAS.forEach(nomeAba => {
    ...
    const semDados = [];
    corpo.forEach((row, i) => {
      if (temDados(row)) comDados.push({ row: row, ordem: i });
    ...
    const novoCorpo = comDados.map(x => x.row).concat(semDados);
    ...
    abasOrdenadas++;
    detalhe.push(nomeAba + ": " + comDados.length + " linhas ordenadas");
  });
  ...
  ABAS.forEach(nomeAba => {
    ...
    for (let r = primeiraLinhaDados; r < disp.length; r++) {
      ...
      if (iL != null) {
        ...
        if (orig && !/^[-–—\s]*$/.test(orig)) {
          const novo = padronizarLessons(orig, "L");
          if (novo !== orig) {
      ...
      if (iS != null) {
        ...
        if (orig && !/^[-–—\s]*$/.test(orig)) {
          const novo = padronizarLessons(orig, "S");
          if (novo !== orig) {
